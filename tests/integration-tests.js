    ...
    async getCredentials() {
        try {
            const apiKey = execSync(
                `KUBECONFIG=${this.kubeconfig} kubectl get secret livekit-credentials -o jsonpath="{.data.api-key}" -n livekit | base64 -d`,
                { encoding: 'utf8' }
            ).trim();
            
            const apiSecret = execSync(
                `KUBECONFIG=${this.kubeconfig} kubectl get secret livekit-credentials -o jsonpath="{.data.api-secret}" -n livekit | base64 -d`,
                { encoding: 'utf8' }
            ).trim();
