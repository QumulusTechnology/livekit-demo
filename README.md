...
### 1. LiveKit Frontend
- **URL**: https://livekit-frontend.livekit-demo.cloudportal.app
...
### 3. LiveKit Ingress
- **RTMP URL**: rtmp://ingress.livekit-demo.cloudportal.app:1935
- **WHIP URL**: https://ingress.livekit-demo.cloudportal.app:8080
...
kubectl get secret livekit-credentials -o jsonpath="{.data.api-key}" -n livekit | base64 -d
kubectl get secret livekit-credentials -o jsonpath="{.data.api-secret}" -n livekit | base64 -d
...
```bash
# Test command used (credentials retrieved from K8s secrets)
kubectl run livekit-load-test --rm -i --tty --image=livekit/livekit-cli \
  --command -- /bin/bash -c "
  livekit-cli load-test \
    --url wss://livekit.livekit-demo.cloudportal.app \
    --api-key \$(kubectl get secret livekit-credentials -o jsonpath='{.data.api-key}' -n livekit | base64 -d) \
    --api-secret \$(kubectl get secret livekit-credentials -o jsonpath='{.data.api-secret}' -n livekit | base64 -d) \
    --room testroom \
    --publishers 100 \
    --subscribers 700 \
    --duration 5m \
...
│   ├── livekit-frontend/   # React frontend configuration
...
kubectl get app livekit-frontend -n argocd
...
- `newtrivoh` - Trivoh frontend application
